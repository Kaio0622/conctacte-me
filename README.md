...
**`ADMIN_WHATSAPP` deve conter o número que recebe os avisos de novos prestadores.** Use o formato E.164, somente dígitos (por exemplo, `5562991522038` para +55 62 99152-2038). Troque pelo
...
grade passa a 3 colunas. Tema claro, escuro ou padrão do dispositivo pelo seletor no topo, também disponível no painel administrativo. A preferência fica salva neste navegador e acompanha mudanças do sistema no modo automático. Campos com
...
### Verificação isolada das correções

Execute `npm test` para criar dados de demonstração em uma pasta temporária e executar os 55 testes de fluxo e as regressões de chat, permissões, validação e localização. O comando não altera os dados e fotos reais. As pastas temporárias são preservadas para diagnóstico.

O seletor **Tema** está disponível no topo do app e do painel administrativo para clientes, prestadores e administradores. Oferece **Claro**, **Escuro** e **Padrão do dispositivo**. A preferência é deste navegador, compartilhada entre abas, e não é sincronizada entre aparelhos.

O chat usa um cursor por ID para não perder mensagens criadas no mesmo milissegundo, mantém a ordem e evita duplicação visual, atualiza o status do pedido, encerra o polling ao sair da tela e preserva o texto quando o envio falha. O administrador pode consultar para moderação sem marcar mensagens como lidas e sem enviar em nome de um participante.

O teste opcional de interface está em `scripts/test-browser.js`: com Playwright e Chrome disponíveis, execute `npm test` com `TEST_BROWSER=1`. Ele verifica dois perfis, entrega de mensagens, falha de envio, encerramento remoto, navegação e temas em viewport mobile e desktop.


### Banco Supabase

O backend agora aceita `DB_DRIVER=supabase`. Consulte [a configuração e os limites da integração](supabase/README.md). Execute `npm run supabase:check` para verificar a conexão. Os testes e o seed permanecem isolados do banco remoto.
